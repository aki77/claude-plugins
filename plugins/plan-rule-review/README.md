...
4. Claude 本体は `plan-rule-review-run` スキルを起動し、その手順に従う:
   0. **トリアージ**: まず安価で高速な **Haiku モデル** のサブエージェントが、プランが「レビューを省略してよいほど自明・単純か」だけを判定する。自明・単純（`TRIAGE: SKIP`）なら本体がマーカーを付けて即 `ExitPlanMode` し、以降のフルレビューをスキップする（ステップ 2 で即許可される）。それ以外（`TRIAGE: REVIEW`）は次のフルレビューへ進む。判定は**保守的**（少しでも迷えばレビューへ回す）で、誤スキップによるルール違反の見逃しを抑える
   1. プラン全文から変更対象ファイルパスを抽出する
   2. `collect-plan-rules.mjs` にそのファイルリストを渡し、該当する `CLAUDE.md` / `.claude/rules/` ファイルのパス一覧だけを JSON で取得する（**glob 照合を機械的に行い、該当ルールを決定論的に絞り込む**）
   3. レビュー専用のサブエージェント（general-purpose）を Task ツールで 1 つ起動する。サブエージェントは**絞り込み済みのパスのファイルのみ**を読み、プランのルール準拠を判定して、結果（`VERDICT: PASS` または `VERDICT: FAIL` ＋違反リスト）をテキストで本体に返す
...
- 自明・単純なプランのフルレビューを省くトリアージ段階は Haiku によるプラン本文の性質判定であり、ルールファイルを読まない。保守的（迷えばレビュー）に設計しているが、誤って `TRIAGE: SKIP` と判定しフルレビューを省く可能性は理論上残る
