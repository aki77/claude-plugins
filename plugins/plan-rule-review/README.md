...
4. Claude は指示に従い、`CLAUDE.md` と `.claude/rules/` を読み込んでプランのルール準拠をレビューする
5. 違反があればプランを修正し（マーカーは付けずに）、再度 `ExitPlanMode` を呼ぶ
6. 違反がなければプラン末尾にマーカーを追記して `ExitPlanMode` を呼ぶ → ステップ 2 で即座に許可される（違反のないプランは 1 回のレビューで通過する）
...
- 違反なしを示すマーカーは Claude 自身が付けるため、Claude が誤って違反の残るプランにマーカーを付けると早期通過してしまう（上記「レビューの質は Claude 本体に依存する」の延長線上の制限）
