...
このプラグインは `ExitPlanMode` を `PreToolUse` フックで捕捉し、`plan-rule-review-run` スキルを起動してプランがプロジェクトルールに違反していないかをレビューさせます。違反があればプランを修正させてから実装に進ませることで、実装着手前にルール準拠を担保します。

レビュー観点は **プロジェクトルール準拠チェックのみ** です（バグ・一般的なコード品質・スタイルは対象外）。

## しくみ

`PreToolUse(ExitPlanMode)` フックと `plan-rule-review-run` スキルが以下の流れで動作します：
...
3. マーカーがなく、セッションあたりのレビュー回数が上限未満なら、`ExitPlanMode` を deny（拒否）し、「`plan-rule-review:plan-rule-review-run` スキルを実行せよ」という指示とプラン全文を Claude の context に注入する
4. Claude 本体は `plan-rule-review-run` スキルを起動し、その手順に従う:
   1. プラン全文から変更対象ファイルパスを抽出する
   2. `collect-plan-rules.mjs` にそのファイルリストを渡し、該当する `CLAUDE.md` / `.claude/rules/` ファイルのパス一覧だけを JSON で取得する（**glob 照合を機械的に行い、該当ルールを決定論的に絞り込む**）。グローバルルール（ルート直下 `CLAUDE.md`、および `paths` 未指定＝全ファイル適用の `.claude/rules/` ルール）は常に除外し、ディレクトリ階層由来の `CLAUDE.md` と `paths` が変更対象ファイルに一致したルールのみを返す
   3. 絞り込み結果が空（該当ルール0件）なら、サブエージェントを起動せず本体がプラン末尾にマーカーを付けて即 `ExitPlanMode` する。該当ルールが1件でもあれば、レビュー専用のサブエージェント（general-purpose）を Task ツールで 1 つ起動する。サブエージェントは**絞り込み済みのパスのファイルのみ**を読み、プランのルール準拠を判定して、結果（`VERDICT: PASS` または `VERDICT: FAIL` ＋違反リスト）をテキストで本体に返す
...
レビュー判断は、プランを書いた本体とは独立した文脈のサブエージェントに委譲します。これにより、本体が自分の書いたプランを甘く自己レビューしてしまうバイアスを軽減します。どのルールファイルが該当するかは `collect-plan-rules.mjs` が `paths` frontmatter の glob 照合で機械的に絞り込むため、サブエージェントは渡されたパスのファイルだけを読めばよく、ルールファイル全体を列挙・照合する必要がありません。本体はサブエージェントの `VERDICT: PASS` 判定を受け取ったときのみマーカーを転記します。

### カスタマイズ

レビュー手順・判定観点・出力形式・サブエージェントへの指示文はすべて [`skills/plan-rule-review-run/SKILL.md`](skills/plan-rule-review-run/SKILL.md) に Markdown で記述されています。レビューの挙動を変えたい場合は、フックスクリプト（`.mjs`）ではなくこの `SKILL.md` を編集してください。該当ルールの絞り込みロジックは [`skills/plan-rule-review-run/scripts/collect-plan-rules.mjs`](skills/plan-rule-review-run/scripts/collect-plan-rules.mjs) にあります。
...
- `node` v22+ が `PATH` 上にあること（`plan-rule-review-run` スキルの `collect-plan-rules.mjs` が `node:fs/promises` の `glob` を使用するため）

## 既知の制限

- レビューの質はサブエージェントに依存する（フックはレビュー指示の注入とレビュー回数の管理のみを行う）
- グローバルルール（ルート直下 `CLAUDE.md`、および `paths` 未指定＝全ファイル適用の `.claude/rules/` ルール）は常にレビュー対象から除外される（確実に読み込み済みでプランに反映されている可能性が高いため）。そのため、これらのグローバルルールに関わる違反はこのプラグインでは検出されない
...
- `.claude/rules/` の `paths` による該当判定は `collect-plan-rules.mjs` が glob 照合で機械的に行うが、その入力となる変更対象ファイルの抽出は Claude 本体がプランから推測する（プラン段階では確定した diff が存在しないため）。抽出が不正確だと絞り込みも不正確になる
