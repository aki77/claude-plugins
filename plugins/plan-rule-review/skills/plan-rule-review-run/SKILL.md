...
>
> レビューを **省略してよい**（SKIP）のは、次のような明らかに自明・単純なプランに限る:
> - 誤字・タイポ修正、コメント・ドキュメント文言の微修正のみ
> - 単一行・ごく小さな自明な変更のみ
> - コード変更を一切伴わない（質問への回答・調査結果の提示のみ）プラン
>
> それ以外（新規ファイル作成、複数ファイルにまたがる変更、ロジックの追加・変更、依存の追加、設定変更など実装を伴うもの）はすべて「レビュー必要」(REVIEW) とする。
>
> レビュー対象のプラン全文:
> （ここにプラン全文をそのまま埋め込む）
>
