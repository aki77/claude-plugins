...
2. **サマリエージェントを起動する。** Task/Agent ツールで Sonnet のサブエージェントを起動し、以下の情報から変更内容のサマリ（変更の意図・全体像・主要な変更点）を返させる。ステップ1と並列で実行してよい（ステップ1の結果に依存しない）。
   ...
   このサマリの出力は以下の2箇所で消費する（宙に浮かせない）:
   1. **ステップ3のエージェント4の入力**として渡す（後述。エージェント4のみに渡し、他のエージェントには渡さない）。
   2. **ステップ7のターミナル最終レビュー結果、および `--comment` 指定時のステップ10-3のレビュー本文**に載せる「変更概要」の材料。
...
- **サマリエージェント（ステップ2）が失敗した場合の縮退動作**: エージェント4はサマリなし（PRタイトル・説明文のみ）で起動してよく、ステップ7/10 の「変更概要」は PRタイトル・説明文で代替する。**サマリ失敗でレビュー全体を止めないこと。**

3. 5つのエージェントを並列で起動し、変更内容を独立してレビューする。各エージェントは、課題の説明と指摘理由（例: 「CLAUDE.md準拠」「バグ」「REVIEW.md準拠」）を含む課題リストを返す。**diff を取得するエージェント（3・4・5）には `gh pr diff <PR> $(jq -r '.excludeArgs.gh[]' "$CTX")`（レビュー対象外ファイルを除外した diff）を使わせること。** エージェント4はステップ2のサマリを入力に使うため、サマリエージェントの完了後に起動する（他の4エージェントはサマリに依存せず、ステップ2と並列に起動してよい）。各エージェントの役割は以下の通り:
   ...
   エージェント4: バグ検出（Opus、エージェント3と並列。**ステップ2のサマリを入力に持つ唯一のエージェント**）
   導入されたコードに存在する問題（セキュリティ問題、ロジック誤り、クロスファイル整合性の崩れなど）を探す。エージェント3と異なり、変更コードの**周辺コンテキスト**まで読み込んでよい。**入力にはステップ2のサマリエージェント出力（変更の意図・全体像）を含めること。** 全体像を踏まえることでクロスファイル整合性・意図と実装のズレを検出しやすくなり、誤検知も減る。具体的には以下の確認のためにdiff外のファイルを参照することを推奨する:
   ...
   **重要: 高シグナルな指摘のみを対象とする。** 以下に該当する指摘のみを行うこと:
   ...
   各サブエージェントには、PRタイトルと説明文を併せて渡す。これにより著者の意図を踏まえたレビューが可能になる。**ステップ2のサマリを渡すのはエージェント4のみ**とし、エージェント1・2・3・5には渡さない（ルール準拠系・REVIEW.md準拠系は担当ファイルへの局所的な違反照合であり全体意図は判断軸に入らないため、また diff限定のエージェント3は役割上追加コンテキストを持たないため）。
