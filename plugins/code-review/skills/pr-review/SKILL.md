...
| **tiny時のサマリエージェント** | **起動しない**（`tier` が `tiny` のとき）。著者意図は PRタイトル・説明文で代替でき（**サマリ失敗時の縮退**と同経路）、クラスタは validate-clusters.mjs が tier 縮退で単一化するため、ステップ2のサマリ+クラスタ分割エージェントを省略する。`clusters` は `[]` を `Write` してステップ2b に通す |
