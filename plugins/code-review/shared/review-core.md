...
- **並列サブエージェント（ステップ2・3・6）は同期的に起動して結果を回収する**（バックグラウンド起動＋通知待ちに依存しない。詳細は後述「並列サブエージェントの起動・完了回収」）。
...
3. レビューエージェントを並列で起動し、変更内容を独立してレビューする。**各エージェントの出力は finding 配列の fenced JSON のみとする**（下記「構造化出力」を厳守。課題0件なら `[]`）。diff を取得するエージェント（3・4・5）には `git diff $(jq -r '.diffArgs[]' "$CTX") $(jq -r '.excludeArgs.git[]' "$CTX")`（レビュー対象外ファイルを除外した diff）を使わせること。各エージェントの起動タイミングは **ステップ3の起動タイミング**（モード別パラメータ表）に従う。**エージェント4はステップ2b の検証済み `clusters` を入力に使うため、ステップ2b の完了後にクラスタ数だけ並列起動する。** 各エージェントの役割は以下の通り:
   ...
   **構造化出力（全レビューエージェント共通・厳守）:** 各エージェントは自身が発見した課題を、次のスキーマの **finding オブジェクトの配列を fenced JSON（```json ... ```）で1つだけ**返すこと。課題が無ければ空配列 `[]` を返す。**説明文・前置き・箇条書きなどのテキストを混ぜず、fenced JSON ブロックのみを返す。**
   ...
   - `existingCode`: **コメントを貼る位置のアンカー**。diff 中にそのまま存在する連続した数行を**逐語コピー**する（行番号はスクリプトが確定するので書かない）。次を厳守:
     ...
     - suggestion で**行を削除する修正になりうる箇所は、削除したい行と残したい行の両方をこの範囲に含める**（例: 31行目のコメントを消して32行目の `def` を残すなら、その2行を `existingCode` に含める）。範囲がずれると後段で suggestion を付けられなくなる。
...
- **並列サブエージェント（ステップ2のサマリ・ステップ3のレビューエージェント・ステップ6の検証エージェント）は同期的に起動して結果を回収すること。`run_in_background: true` で起動し、harness の完了通知（task-notification）や `SendMessage` での逐次回収に依存してはならない。** 理由: headless（非対話）実行ではメインエージェントがツール呼び出しなしのテキストを返した時点でターンが確定し、保留中のバックグラウンドタスクの完了を待たずにセッションが `completed` として終了しうる（＝完了通知が届く前にレビューが打ち切られる）。対話 CLI でも headless でも同一挙動になるよう、常に同期回収に統一する。
- **並列性は「1つのメッセージ内で複数の Agent 呼び出しを同時に発行し、その全結果を同一ターンで受け取る」ことで担保する。** これで並列実行と同期回収が両立する。バックグラウンド起動・通知待ち・`ScheduleWakeup` 等による自己ポーリング・サブエージェントの出力 JSONL／タスクファイルの覗き見による完了判定は、いずれも不要かつ禁止（同期回収では原理的に発生しない）。
