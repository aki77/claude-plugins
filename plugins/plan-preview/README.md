...
> 環境変数名は `plan-workflow` プラグインから分離した経緯により `PLAN_WORKFLOW_*` のままです（後方互換のため維持）。
...
本プラグインは [Plan モードのループを改善する記事](https://zenn.dev/k_yoshiya/articles/claude-code-plan-mode-loop) のプラン確認手法を踏まえていますが、プラン確定シグナル（`<!-- render -->` マーカー）と HTML render 機構は採用せず、`PermissionRequest(ExitPlanMode)` の mo プレビューでプラン確認を代替しています。

## 関連プラグイン

- [`plan-workflow`](../plan-workflow) — Plan モードの運用ルール（要件インタビュー・実装の Agent 委譲・モデル振り分け）を注入する。mo プレビュー機能はもともと plan-workflow に含まれていたが、関心の分離のため本プラグインに切り出した。両者は独立して有効化でき、併用時はルール注入と mo プレビューがそれぞれ別フックから動作する。
