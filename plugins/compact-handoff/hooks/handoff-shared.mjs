...
export const MAX_SEGMENT_CHARS = 2_000_000;
...
// segmentText は前回 PreCompact 以降に増えた生ログ区間（JSONL行をそのまま連結したもの）。
...
// state ファイル（前回処理済み行数と pending 引き継ぎ内容）の永続化先を決める。
...
// state を読む。ファイルが無い・壊れている場合は空オブジェクト（fail-safe）。
...
// state を書く。書き込み失敗は無視
// （次回また同じ内容を処理し直すだけで実害は小さいため、フック本体を止めない）。
...
if (
  ...
  test("writeState/readState: 書いて読み直すラウンドトリップ", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "compact-handoff-test-"));
    const p = path.join(dir, "nested", "session-1.json");
    writeState(p, { lastProcessedLineCount: 5, pending: { status: "no_gap" } });
    assert.deepEqual(readState(p), { lastProcessedLineCount: 5, pending: { status: "no_gap" } });
    fs.rmSync(dir, { recursive: true, force: true });
  });
}
