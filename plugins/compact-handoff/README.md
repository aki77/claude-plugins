...
5. 出力を検証し、問題なければ `hookSpecificOutput.additionalContext` として返し、次のターン開始前に会話へ強制的に読み込ませる。同時に `systemMessage` で同じ内容をユーザーの画面にも表示する（抽出に失敗した場合や該当情報が無かった場合も、`systemMessage` のみでその旨をユーザーに通知する。モデルには渡らない）
